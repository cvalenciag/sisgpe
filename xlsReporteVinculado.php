...
$qryMalla = $conn->query("SELECT descripcion as nomCarrera FROM perfilegresado LEFT JOIN carrera USING (idCarrera) WHERE
                          idCarrera='$idCarrera' AND fAprobacionMalla='$fechaMalla' AND fAprobacionPerfil='$fechaPerfil'")or die(mysqli_error($conn));
  ...
  <thead>
    <tr>
      <td colspan="3" rowspan="5" align="center">
        <p>
          <br>
          <!-- <font color="#9F4A95" size="5" face="Times New Roman"> -->
          <font size="5" face="Times New Roman" color=" <?php echo $idCarrera==11 ? '#30a04a' : ($idCarrera == 12 ? '#7a183b' :
          ($idCarrera==13 ? '#e04544' : ($idCarrera==14 ? '#4c3278' :
          ($idCarrera==21 ? '#1e9bd1' :
          ($idCarrera==31 ? '#f0a02b' :
          ($idCarrera==32 ? '#f9d126' :
          ($idCarrera==41 ? '#9c4a93' : '#1fa9aa') ) ) ) ) ) ) ?>  ">
            Malla Curricular: <br> <b><?php echo $result['nomCarrera']; ?></b></font>
          <br>
          <font size="3.5" face="Times New Roman"><b>(Ciclo - Curso)</b></font>
          <br>
          <font size="3.5" face="Times New Roman"><b>Fecha de Perfil de egreso:</b></font>
          <br>
          <font size="3.5" face="Times New Roman"><b><?php echo $fechaPerfil ?></b></font>
          <!-- <br><br><br> -->
...
?>
  ...
  <?php
    $qryCursos = $conn->query("SELECT * FROM perfilegresado p LEFT JOIN detalle_malla dm USING (idCarrera)
                              LEFT JOIN curso c ON
                              (c.idCurso=dm.idCurso) LEFT JOIN departamento dep ON (dep.idDepartamento = c.idDepartamento) WHERE p.idCarrera='$idCarrera' AND fAprobacionMalla='$fechaMalla' AND fAprobacionPerfil='$fechaPerfil' ORDER BY ciclo , nombreCurso") or die(mysqli_error($conn));


    $qryCount = $conn->query("SELECT COUNT(*) as total FROM perfilegresado p LEFT JOIN detalle_malla dm USING (idCarrera)
                              LEFT JOIN curso c ON (c.idCurso=dm.idCurso) WHERE p.idCarrera='$idCarrera' AND fAprobacionMalla='$fechaMalla' AND fAprobacionPerfil='$fechaPerfil' ORDER BY ciclo , nombreCurso") or die(mysqli_error($conn));
