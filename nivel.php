	...
	<?php require("head.php"); ?>
		<div class = "container-fluid">
			...
			<div class = "col-lg-9 well" style = "margin-top:110px;background-color:#fefefe;">
				<div class = "alert alert-jcr">Rúbricas / Niveles de desempeño</div>
					...
					<label>Descargar</label>
						<table id = "table" class = "table table-bordered table-hover" style="width:100%">
							...
							if($_SESSION['rol_id']==1)
                                                        ...
                                                        $rol=" where estado=1";
								$q_admin = $conn->query("SELECT * FROM `nivel`".$rol) or die(mysqli_error($conn));
							...
							?>
								<tr class = "target">
									...
									<div style = "float:right;">
                                                                        <a href = "#" class = "btn btn-editar eadmin_id" value = "<?php echo $f_admin['idNivel']?>">
                                                                        <span class = "glyphicon glyphicon-edit" title="Editar registro"></span> </a>
									<a href = "#" class = "btn <?php echo $btnclas?> deladmin_id" value = "<?php echo $f_admin['idNivel'].'&estado='.$estado?>">
					...
					<div id = "admin_form" style = "display:none;">
						...
						<div class = "col-lg-6">
							<form id = "formnivel" method = "POST" action = "nivelGrabar.php" enctype = "multipart/form-data">
								<div class = "form-group">
								<label class = "titulo">Registre información sobre un nivel de desempeño a la vez.</label>
								</div>

								<div class = "form-group">

									<label>Nivel:</label>
									<textarea required = "required" maxlength = "20" rows="1" name = "descripcion" class = "form-control"></textarea>
								</div>
	...
	<script type = "text/javascript">
		$(document).ready(function(){
			...
			$("#table").on("click", ".deladmin_id", function(){
				...
				setTimeout(function(){
					window.location = 'nivelEliminar.php?admin_id=' + $admin_id;
				}, 1000);
			});
			$("#table").on("click", ".eadmin_id", function(){
				...
				$('#admin_table').fadeOut();
				$('#add_admin').hide();
				$('#edit_form').load('nivelCargar.php?admin_id=' + $admin_id);
			});
		});
	</script>
</html>
