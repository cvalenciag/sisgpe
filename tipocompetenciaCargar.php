<?php
	...
	$qedit_admin = $conn->query("SELECT * FROM `tipocompetencia` WHERE `idTipo` = '$_REQUEST[admin_id]'") or die(mysqli_error($conn));
	$fedit_admin = $qedit_admin->fetch_array();
?>
<div class = "col-lg-3"></div>
<div class = "col-lg-6">
	<form method = "POST" action = "tipocompetenciaEditar.php?admin_id=<?php echo $fedit_admin['idTipo']?>" enctype = "multipart/form-data">
		<div class = "form-group">
								<label class = "titulo">Registre información sobre UN tipo de competencia a la vez.</label>
								</div>
		<div class = "form-group">
			<label>Tipo de competencia:</label>
			<textarea rows="1" required = "required" name = "descripcion" class = "form-control" maxlength = "80"><?php echo $fedit_admin['descripcion']?></textarea>
		</div>	
		<div class = "form-group">
