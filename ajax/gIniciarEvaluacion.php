...
<div class="col-lg-12" style="margin-top:15px;">
  <table style="width:100%;">
    <tr>
      <td style="width:250px;"><b>EVALUADOR:</b></td>
      <td>
        <select name="depAcad" id="idEvalSave" required="required" class="form-control" style="width:300px;">
          <option value="" selected="selected">Seleccione una opción</option>
          <?php
            $qryEvaluador = $conn->query('SELECT * FROM evaluador WHERE estado=1 ORDER BY nomEvaluador') or die(mysqli_error($conn));

            foreach ($qryEvaluador as $evaluador)
            {
          ?>
              <option value="<?php echo $evaluador['idEvaluador'] ?>">
                <?php echo $evaluador['nomEvaluador'].' '.$evaluador['apeEvaluador'] ?>
              </option>
          <?php
            }
         ?>
