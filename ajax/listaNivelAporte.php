...
?>
	<table id="table" class="table table-bordered table-hover" style="width:100%">
		...
		<tbody>
			<?php
				$q_admin = $conn->query("SELECT dm.idCurso, c.nombreCurso, dm.ciclo, d.descripcion, c.tipoCurso, dm.aol FROM detalle_perfilegresado_curso dp 
				LEFT JOIN detalle_malla dm ON (dm.idCurso = dp.idCurso)
				LEFT JOIN perfilegresado p ON (p.idPerfilEgresado = dp.idPerfilEgresado)
				LEFT JOIN carrera ca ON (ca.idCarrera=dm.idCarrera)
				LEFT JOIN curso c ON (c.idCurso = dm.idCurso)
				LEFT JOIN departamento d ON (d.idDepartamento = c.idDepartamento)
				WHERE p.idPerfilEgresado='$idPerfil' AND ca.idCarrera='$idCarrera' and dm.eliminado = 0 order by dm.ciclo, c.nombreCurso") or die(mysqli_error($conn));
