	...
	<?php require("head.php"); ?>
		<div class = "container-fluid">
			...
			<div class = "col-lg-9 well" style = "margin-top:110px;background-color:#fefefe;">
				<div class = "alert alert-jcr">Organización / Departamentos</div>
						...
						<table id = "table" class = "table table-bordered table-hover" style="width:100%">
							...
							<?php
							  if($_SESSION['rol_id']==1)
                                                        ...
                                                        $rol=" where estado=1";
								$q_admin = $conn->query("SELECT * FROM `departamento`".$rol) or die(mysqli_error($conn));
								while($f_admin = $q_admin->fetch_array()){
									
							?>	
								<tr class = "target">
									<td><?php echo $f_admin['codUpDepartamento']?></td>
...
<div style = "float:right;">									
 <a href = "#" class = "btn btn-editar eadmin_id" value = "<?php echo $f_admin['idDepartamento']?>">
                                                                        <span class = "glyphicon glyphicon-edit" title="Editar registro"></span> </a>
									<a href = "#" class = "btn <?php echo $btnclas?> deladmin_id" value = "<?php echo $f_admin['idDepartamento'].'&estado='.$estado?>">
					...
					<div id = "admin_form" style = "display:none;">
						...
						<div class = "col-lg-6">
							<form id = "formdepartamento" method = "POST" action = "departamentoGrabar.php" enctype = "multipart/form-data">
							<div class = "form-group">
									<label>Código UP Departamento Académico:</label>
									<input type = "text" required = "required" name = "codUpDepartamento" class = "form-control" maxlength = "2" />
								</div>
							<div class = "form-group">
									<label>Nombre corto:</label>
									<input type = "text" required = "required" onKeyUp="this.value=this.value.toUpperCase();" name = "nombreCorto" class = "form-control" maxlength = "10" />
								</div>								
								<div class = "form-group">
									<label>Departamento Académico:</label>
									<textarea required = "required" rows="2" placeholder = "Escriba con mayúsculas y minúsculas" name = "descripcion" class = "form-control" maxlength = "80" ></textarea>
								</div>
								<div class = "form-group">
	...
	<script type = "text/javascript">
		$(document).ready(function(){
			...
			$("#table").on("click", ".deladmin_id", function(){
				...
				setTimeout(function(){
					window.location = 'departamentoEliminar.php?admin_id=' + $admin_id;
				}, 1000);
			});
			$("#table").on("click", ".eadmin_id", function(){
				...
				$('#admin_table').fadeOut();
				$('#add_admin').hide();
				$('#edit_form').load('departamentoCargar.php?admin_id=' + $admin_id);
			});
		});
	</script>
</html>